...
if __name__ == "__main__":
  ...
  for i in range(0,1):
    ...
    INPUT_PATH+= " /user/yuebin/hadoop/mining_user_intent/"+str(tmp_date).split(' ')[0].replace('-','')+".st"
  OUTPUT_PATH='/user/yuebin/hadoop/mining_user_intent_merge/%s.st' %(today.replace('-',''))
  ...
  command =  hadoop_home_path+"/bin/hadoop jar " + hadoop_streaming_file + \
          ...
          " -file " + binpath + "merge_reducer.py " + \
          " -file " + confpath + "config.py"  + \
          " -jobconf mapred.reduce.tasks=100 " + \
          " -jobconf mapred.job.name=\"mining_user_intent_merge_"+today+"\""  +\
  ...
  if ret!=0:
    alert("mining_user_intent_merge job faild")
